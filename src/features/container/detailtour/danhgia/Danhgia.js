...
function Danhgia(props) {
  ...
  const load = useSelector((state) => state.binhluan.loading);
  ...
  const onSubmit = (e) => {
    ...
    if (binhluan.length === 0 || binhluan.length > 10) {
      if (binhluans.find((x) => x.tourId === +tourId && x.userId === +userId)) {
        ...
        dispatch(
          updatebinhluan({ idsua, tourId, binhluan, userId, star, status })
        );
        setTimeout(() => {
          actionbinhluan();
        }, 500);
        ...
        <div className="mb-5">
          ...
          <form action="" method="post" onSubmit={onSubmit}>
            ...
            <div className="position-relative">
              <Button htmlType="submit" type="primary" className="btn-dg mt-2">
                Đánh giá
              </Button>
            </div>
          </form>
